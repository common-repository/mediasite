...
function mediasite_get_iv_by_index($index) {
	$pluginOptions = get_option('mediasite_options');

	return $pluginOptions['iv' . $index . '_template'];
}
...
function mediasite_generate_markup($mediasiteMarkup) {
	...
	if ($id != null && $server != null && $serverFieldIndex > 0) {
		...
		if ($auth != null && $auth === 'true') {
			...
			$client = new ExternalAccessClient($server . '/Mediasite/Services60/Edas.svc?WSDL');
			
			if (!isset($_SESSION['MediasiteAPITicket_' . $server])) {
				$loginResponse = $client->Login(
												mediasite_get_username_by_index($serverFieldIndex),
												mediasite_decrypt(mediasite_get_password_by_index($serverFieldIndex), mediasite_get_iv_by_index($serverFieldIndex))
				);
...
function mediasite_options_validate($input){
	...
	foreach($arrayKeys as $fieldName) {
		...
		if (strpos($fieldName, 'password') === 0) {
			$input[$fieldName] = Encryption::encrypt($input[$fieldName]);
		}
	}
...
function mediasite_password1_field() {
	$options = get_option('mediasite_options');
	$password = (isset($options['password1_template'])) ? Encryption::decrypt($options['password1_template']) : '';
...
function mediasite_password2_field() {
	$options = get_option('mediasite_options');
	$password = (isset($options['password2_template'])) ? Encryption::decrypt($options['password2_template']) : '';
